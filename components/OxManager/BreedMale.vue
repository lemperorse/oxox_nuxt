...
export default class Food extends Vue {
  ...
  async getEnv() {
    ...
    this.lists = await Core.getHttp(
      `${apis}/breed/male/breed/?ox=${this.currentId}`
    );
    this.response = true;
  }
  ...
  async saveData() {
    ...
    let store = await Core.postHttp(`${apis}/breed/male/breed/?ox=${this.currentId}`, this.form);
    if (store.id) {
      await Core.putHttp(`/api/v1/ox/ox/${this.currentId}/`, { status: "สำเร็จ" });
  ...
  async updateData() {
    this.form.ox = this.currentId;
    let store = await Core.putHttp(`${apis}/breed/male/breed/${this.form.id}/`, this.form);
    if (store.id) {
      await Core.putHttp(`/api/v1/ox/ox/${this.currentId}/`, { status: "สำเร็จ" });
      alert("บันทึกข้อมูลสำเร็จแล้ว");
      await this.getEnv();
  ...
  async removeData() {
    ...
    if (check) {
      ...
      await Core.deleteHttp(`${apis}/breed/male/breed/${this.form.id}/`);
