...
<div>
    ...
    <v-toolbar color="transparent" flat>
      <h2 class="text-base font-bold">อาหารผสมครบส่วน (TMR)</h2>
      <v-spacer></v-spacer>
      ...
      <div @click="(form = list) && (dialog = true)" v-for="list,index in lists" :key="index">
            <Core-Menu :name="list.tmr_mix" icon="food3.png" :text="list.fedding_system"></Core-Menu>
        </div>
...
export default class FoodRough extends Vue {
    ...
    async saveData(){
      this.form.ox = this.currentId
        let data = await Core.postHttp(`/api/v1/ox_manager/food_tmr/`,this.form)
        if(data.id)
        {
    ...
    async updateData(){ 
        let data = await Core.putHttp(`/api/v1/ox_manager/food_tmr/${this.form.id}/`,this.form)
        if(data.id)
        {
          alert('บันทึกข้อมูลสำเร็จ')
          await this.getEnv();
