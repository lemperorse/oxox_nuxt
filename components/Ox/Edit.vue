...
<div>
    <v-container>
        <form @submit.prevent="saveData()" v-if="response">
            ...
            {{form.sex}}
            <v-text-field v-if="form.sex == 'โคขุน'" dense class="p-2" type="date" label="วันที่เข้าขุน" v-model="form.fatten_date" prepend-inner-icon="mdi-calendar-star" />
          ...
          <div class="flex  p-2">
                ...
                <v-btn v-if="form.sex == 'โคขุน'"  class="-mt-2" small fab @click="form.weight = form.breast*2.23" color="primary">คำนวน</v-btn>
            </div>
           ...
           <v-text-field v-if="form.sex == 'โคขุน'"  dense class="p-2" type="number" label="น้ำหนักเข้าขุน (กิโลกรัม)" v-model="form.weight" prepend-inner-icon="mdi-scale" />
