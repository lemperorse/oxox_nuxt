...
<div>
    ...
    <v-container>
        <form @submit.prevent="saveData()">
            <v-text-field dense class="p-2" label="ชื่อโค" v-model="form.name" />
            ...
            <v-select dense class="p-2" :items="choices.gene" item-text="name" item-value="id" label="พันธุ์" v-model="form.gene_id" />

            <v-select dense class="p-2" :items="choices.gender" item-text="name" item-value="id" label="เพศ" v-model="form.gender_id" />
            <v-text-field dense class="p-2" label="เพศอื่นๆ" v-model="form.gender_ect" />

            <v-select dense class="p-2" :items="choices.origin" item-text="name" item-value="id" label="แหล่งที่มา" v-model="form.origin_id" />
            <v-select dense class="p-2" :items="choices.tooth" item-text="name" item-value="id" label="ฟัน" v-model="form.tooth_id" />
